...
```
  ...
  yaavt.lua join [options] <path> [path] ...               - join a list of file paths into a single media file
```
...
```
  ...
  -o <path>             - set output path (only used by join command and autogenerated if not supplied)
